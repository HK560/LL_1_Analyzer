...
bool LL_1::createLLlist() {
  try {
    ...
    for (auto i = ruleList.begin(); i != ruleList.end(); i++) {
      ...
      QStringList leftRight = nowRule.split("->", QString::SkipEmptyParts);
...
void LL_1::analysis() {
		...
		int nowInputStrSymbolIndex = std::distance(inputString.begin(), itr_nowInputStrChar);
		//判断符号栈顶和当前指向的输入串字符是否相等
		if (nowStackTopSymbol == nowInputStrSymbol) {
			//相等时
			if (nowStackTopSymbol == '#') {//如果都为#则匹配成功结束
				outputStep(nowStackTopSymbol, inputString.mid(nowInputStrSymbolIndex));
		...
		//符号栈顶和当前指向的输入串字符不相等时
			...
			auto itr_inputStrSymbol = LL_Maps[nowStackTopSymbol].find(nowInputStrSymbol);
			...
			if ((itr_stackTopSymbol != LL_Maps.end() 
				...
				QString nowRule = LL_Maps[nowStackTopSymbol][nowInputStrSymbol];//得到规则
				if (!nowRule.isEmpty()) {
					if (nowRule.at(0) != '%') {
						std::reverse(nowRule.begin(), nowRule.end());//逆序字符串
						//推入栈中
						for (auto itr_nowRule = nowRule.begin(); itr_nowRule != nowRule.end(); itr_nowRule++) {
    ...
    for (auto i = ruleMap.begin(); i != ruleMap.end(); i++) {
      ...
      for (auto k = i.value().begin(); k != i.value().end(); k++) {
        ...
        if (nowRule.contains(symbol)) {
          ...
          if (backSym != symbol && nowRule.at(nowRule.size() - 2) == symbol) {
...
void LL_1::createFIRST() {
  ...
  int nowtime = 0;
  while (sum--) {
    for (auto i = nonTerminatorVector.begin(); i != nonTerminatorVector.end();
         i++) {
...
{
    QDebug outputText=qDebug();
	outputText<<"STEP:";
	
	outputText<<"SymbolStackTop:";
	outputText<<stackTopChar;
	outputText<<"nowInputStr:";
	outputText<<nowInputStrChar;
}
  ...
  for (auto i = ruleMap.begin(); i != ruleMap.end(); i++) {  //遍历每一个规则
    ...
    for (auto k = i.value().begin(); k != i.value().end();
      ...
      for (auto t = ownFirstSym.begin(); t != ownFirstSym.end();
        ...
        QChar nowSym = *t;
        if (nowSym != '%' &&
            nowNonTerminatorFIRST.contains(nowSym)) {  //首字符与first匹配
